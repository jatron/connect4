...
class HumanPlayer(Player):
    ...
    def _input_handler(inp, connect4_board):
        ...
        if inp[0] == 'save' and len(inp) == 2:
            ...
            if connect4_board.player1.type is Agents.NetworkPlayer or connect4_board.player2.type is Agents.NetworkPlayer:
...
class NetworkPlayer(Player):
            ...
            if address[0] == self.owner.peer_address:
                ...
                for elm in data:
                    ...
                    if latest_recieved_data is NetworkPlayer.Data.Move:
                        ...
                        logger.debug('Recieved {}:{}.'.format(latest_recieved_data, self.latest_recieved_move))
                    elif latest_recieved_data is NetworkPlayer.Data.Board:
                        self.latest_recieved_board = np.array(elm['content'], dtype=int)
                        logger.debug('Recieved {}:\n{}.'.format(latest_recieved_data, np.array(self.latest_recieved_board, dtype=int)))
                    ...
                    elif latest_recieved_data in (NetworkPlayer.Data.Null, NetworkPlayer.Data.FinishedWin, NetworkPlayer.Data.FinishedLose):
                        pass
    ...
    def __init__(self, peer_address, peer_port=3500, local_port=3500, *args, **kwargs):
        ...
        self.latest_sent_message = [{
            'type': NetworkPlayer.Data.Null.value,
            'content': None
        }]
    ...
    def next_move(self, connect4_board):
        ...
        connect4_board.print_board()
        print(self.color + 'Waiting for player{}\'s response'.format(self.no) + Fore.RESET)
    ...
    def game_finished(self, connect4_board, won:bool):
        ...
        self.server.send(data=self.latest_sent_message, address=self.peer_address, port=self.peer_port)
